...
```mermaid
graph LR
    A[HBOS Operador Open Source] --> B[1. Academia & Cursos]
    A --> C[2. Consultoría B2B]
    A --> D[3. Red de Afiliados]
    A --> E[4. Contenido & Audiencias]
    A --> F[5. Venta de Workflows]

    B --> B1[Curso: Operador de Apps Open Source para Video IA]
    C --> C1[Despliegue de FreeLLMAPI + Ollama privado en empresas]
    D --> D1[Hardware, servidores GPU Cloud, herramientas cloud]
    E --> E1[Parrilla semanal YouTube/TikTok/X monetizada por CPM/Patrocinios]
    F --> F1[Workflows ComfyUI + Scripts FFmpeg + Colecciones Qdrant]
```
